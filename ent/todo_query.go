...
func (tq *TodoQuery) QueryOwner() *UsersQuery {
	query := &UsersQuery{config: tq.config}
...
func (tq *TodoQuery) WithOwner(opts ...func(*UsersQuery)) *TodoQuery {
	query := &UsersQuery{config: tq.config}
...
func (tq *TodoQuery) sqlAll(ctx context.Context) ([]*Todo, error) {
	...
	if query := tq.withOwner; query != nil {
		...
		for i := range nodes {
			if nodes[i].users_todos == nil {
				continue
			}
			fk := *nodes[i].users_todos
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "users_todos" returned %v`, n.ID)
			}
			for i := range nodes {
